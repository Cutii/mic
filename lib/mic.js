...
function mic({
    ...
    function start() {
        ...
        if (usePulseAudio) {
            exec('pulseaudio --check').on('exit', code => {
                if (code !== 0) {
                    return Promise.reject('You choose PulseAudio but daemon is not started.');
                }
            });
        }
        return new Promise((resolve, reject) => {
            try {
                if (usePulseAudio) {
                    ...
                    const format = `${formatEncoding}${bitwidth}${formatEndian}`;
                    audioProcess = spawn(
                        'pacat',
                        [
                            '--rate',
